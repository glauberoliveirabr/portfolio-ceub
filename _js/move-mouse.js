...
const CONFIG = {
    ...
    CURL: 3.0,
    SPLAT_RADIUS: 0.2,   // % do menor lado
    SPLAT_FORCE: 6000,
    SHADING: true,
  ...
  const wrap = (value, min, max) => ((value - min) % (max - min || 1)) + min;
  const scaleByPixelRatio = (n) => Math.floor(n * (window.devicePixelRatio || 1));
  ...
  class Material {
    ...
    setKeywords(keywords){
      let hash = 0; for (const k of keywords) hash += [...k].reduce((h,c)=>((h<<5)-h + c.charCodeAt(0))|0, 0);
  ...
  const advectionShader = compileShader(gl.FRAGMENT_SHADER, `
    ...
    vec4 bilerp(sampler2D sam, vec2 uv, vec2 t){ vec2 st = uv/t - 0.5, iuv = floor(st), fuv = fract(st);
      vec4 a = texture2D(sam, (iuv + vec2(0.5,0.5))*t); vec4 b = texture2D(sam, (iuv + vec2(1.5,0.5))*t);
      vec4 c = texture2D(sam, (iuv + vec2(0.5,1.5))*t); vec4 d = texture2D(sam, (iuv + vec2(1.5,1.5))*t);
  ...
  const vorticityShader = compileShader(gl.FRAGMENT_SHADER, `
    precision highp float; precision highp sampler2D; varying vec2 vUv, vL, vR, vT, vB; uniform sampler2D uVelocity, uCurl; uniform float curl, dt;
  ...
  function createDoubleFBO(w,h, internalFormat, format, type, param){
    ...
    return { width:w, height:h, get texelSizeX(){return fbo1.texelSizeX;}, get texelSizeY(){return fbo1.texelSizeY;},
  ...
  function resizeDoubleFBO(target,w,h, internalFormat, format, type, param){
    ...
    target.width=w; target.height=h; return target;
  }
  ...
  function updateKeywords(){
    const k = []; if (CONFIG.SHADING) k.push('SHADING'); displayMaterial.setKeywords(k);
  }
  updateKeywords();

  function getResolution(resolution){
    let ar = gl.drawingBufferWidth / gl.drawingBufferHeight; if (ar < 1) ar = 1.0/ar;
  ...
  let running = !prefersReduced; // respeita o sistema por padrão

  function calcDeltaTime(){ const now = Date.now(); let dt = (now - lastUpdateTime)/1000; dt = Math.min(dt, 1/60); lastUpdateTime = now; return dt; }
  ...
  function clickSplat(p){ const color = generateColor(); color.r*=10; color.g*=10; color.b*=10; const dx = 10*(Math.random()-0.5); const dy = 30*(Math.random()-0.5); splat(p.texcoordX, p.texcoordY, dx, dy, color); }
  ...
  function ensureSizeAndFbos(){ if (resizeCanvas()) initFramebuffers(); }

  // ====== Loop de animação ======
  function frame(){ if (!running) return; const dt = calcDeltaTime(); ensureSizeAndFbos(); updateColors(dt); applyInputs(); step(dt); render(null); requestAnimationFrame(frame); }

  // ====== Eventos (passive quando possível) ======
  const startOnceMouse = (e)=>{ const p = pointers[0]; const x = scaleByPixelRatio(e.clientX), y = scaleByPixelRatio(e.clientY); if (!prefersReduced && !running){ running = true; frame(); } updatePointerMoveData(p, x, y, generateColor()); window.removeEventListener('mousemove', startOnceMouse); };
  const startOnceTouch = (e)=>{ const t = e.targetTouches; const p = pointers[0]; for (let i=0;i<t.length;i++){ const x = scaleByPixelRatio(t[i].clientX), y = scaleByPixelRatio(t[i].clientY); if (!prefersReduced && !running){ running = true; frame(); } updatePointerDownData(p, t[i].identifier, x, y); } window.removeEventListener('touchstart', startOnceTouch); };

  window.addEventListener('mousemove', startOnceMouse, { passive: true });
  window.addEventListener('mousedown', (e)=>{ const p = pointers[0]; const x = scaleByPixelRatio(e.clientX), y = scaleByPixelRatio(e.clientY); updatePointerDownData(p, -1, x, y); clickSplat(p); }, { passive: true });
  window.addEventListener('mousemove', (e)=>{ const p = pointers[0]; const x = scaleByPixelRatio(e.clientX), y = scaleByPixelRatio(e.clientY); updatePointerMoveData(p, x, y, p.color); }, { passive: true });
  window.addEventListener('touchstart', startOnceTouch, { passive: true });
  window.addEventListener('touchstart', (e)=>{ const t = e.targetTouches; const p = pointers[0]; for (let i=0;i<t.length;i++){ const x = scaleByPixelRatio(t[i].clientX), y = scaleByPixelRatio(t[i].clientY); updatePointerDownData(p, t[i].identifier, x, y); } }, { passive: true });
  window.addEventListener('touchmove', (e)=>{ const t = e.targetTouches; const p = pointers[0]; for (let i=0;i<t.length;i++){ const x = scaleByPixelRatio(t[i].clientX), y = scaleByPixelRatio(t[i].clientY); updatePointerMoveData(p, x, y, p.color); } }, { passive: true });
  window.addEventListener('touchend', (e)=>{ const t = e.changedTouches; const p = pointers[0]; for (let i=0;i<t.length;i++){ updatePointerUpData(p); } }, { passive: true });

  // Inicializa canvas / FBOs e inicia se permitido
  resizeCanvas(); initFramebuffers(); if (!prefersReduced) { running = true; requestAnimationFrame(frame); }

  // ====== Controles ======
  toggleBtn.addEventListener('click', () => {
    running = !running;
    toggleBtn.setAttribute('aria-pressed', String(!running ? true : false));
    toggleBtn.textContent = running ? 'Pausar efeito' : 'Retomar efeito';
    if (running) requestAnimationFrame(frame);
  });
  splatBtn.addEventListener('click', () => { const p = pointers[0]; p.texcoordX = 0.5; p.texcoordY = 0.5; clickSplat(p); });

  // Limpeza ao sair da aba (economia de energia)
  document.addEventListener('visibilitychange', () => { if (document.hidden) { running = false; } else if (!prefersReduced) { running = true; requestAnimationFrame(frame); } });